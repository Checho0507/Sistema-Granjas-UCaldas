...
const ItemInventarioForm: React.FC<Props> = ({ isOpen, onClose, onSave, campos, item }) => {
  ...
  // Sincronizar TODOS los estados cuando cambia item, campos o isOpen
  useEffect(() => {
    if (item) {
      setFechaInventario(
        item.fecha_inventario?.slice(0, 10) ||
        new Date().toISOString().slice(0, 10)
      );
      setCantidadDisponible(item.cantidad_disponible ?? 0);
      setUnidadMedida(item.unidad_medida || '');
      setObservaciones(item.observaciones || '');
      setValores(item.valores || {});
    } else {
      setFechaInventario(new Date().toISOString().slice(0, 10));
      ...
      case 'text':
        return (
          <input
            type="text"
            value={value}
            onChange={(e) => handleCampoChange(campo.nombre_campo, e.target.value)}
            className={common}
      ...
      case 'select':
        return (
          ...
          >
            ...
            {(Array.isArray(campo.opciones) ? campo.opciones : []).map((opt: string) => (
        ...
        return (
          <select
            ...
            onChange={(e) => {
              ...
              if (val === 'true') handleCampoChange(campo.nombre_campo, true);
              else if (val === 'false') handleCampoChange(campo.nombre_campo, false);
              else handleCampoChange(campo.nombre_campo, '');
            }}
            className={common}
          >
            <option value="">Seleccione...</option>
          ...
          <div className="grid grid-cols-2 gap-4 mb-4">
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) => setCantidadDisponible(parseFloat(e.target.value) || 0)}
                className="w-full border rounded p-2"
              />
            </div>
            <div>
          ...
          {campos.sort((a, b) => a.orden - b.orden).map(campo => (
            <div key={campo.id} className="mb-4">
              <label className="block text-sm font-medium mb-1">
                {campo.nombre_campo} {campo.requerido && '*'}
              </label>
              {renderCampo(campo)}
            </div>
          ))}
